...
select product.category_id, count(1) cnt from product group by category_id having cnt > 1 order by cnt desc;
# 4. 综合版, 统计每类商品的总价格, 只统计单价在500以上的商品信息, 且只显示总价在 2000 以上的分组信息, 然后按照总价升序排列, 求出价格最低的那个分类信息.
    select product.category_id, sum(product.price) total_price from product where price > 500 group by category_id having total_price > 2000 order by total_price limit 1;
