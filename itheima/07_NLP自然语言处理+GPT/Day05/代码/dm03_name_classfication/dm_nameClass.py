...
def lstm_predict(x):
    ...
    # 加载训练好的模型
    my_lstm = My_LSTM(input_size=57, hidden_size=128, output_size=18)
    my_lstm.load_state_dict(torch.load('./save_model/ai20_lstm_3.bin'))
    # 实现模型的预测
    with torch.no_grad():
        # 将数据送入模型
...
def gru_predict(x):
    ...
    tensor_x =  line2tensor(x)
    # 加载训练好的模型
    my_rnn = My_GRU(input_size=57, hidden_size=128, output_size=18)
    my_rnn.load_state_dict(torch.load('./save_model/ai20_gru_3.bin'))
    # 实现模型的预测
    with torch.no_grad():
        # 将数据送入模型
        output, hn = my_rnn(tensor_x, my_rnn.inithidden())
